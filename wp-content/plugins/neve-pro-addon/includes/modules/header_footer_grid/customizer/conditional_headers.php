	...
	public function conditional_headers_filtering( $data, $filter_context ) {
		...
		if ( isset( $data[ $mod ]['value']['rules'] ) ) {
			...
			foreach ( $conditions as $custom_layout_slug => $rules ) {
				$custom_layout = self::get_page_by_path( $custom_layout_slug, OBJECT, 'neve_custom_layouts' );
		...
		if ( isset( $data[ $mod ]['value']['themeMods'] ) ) {
			...
			$json_mods = self::$theme_mods_keys;

			foreach ( $to_update as $custom_layout_slug => $value ) {
				if ( $custom_layout_slug === 'default' ) {
					foreach ( $value as $key => $val ) {
						$val = in_array( $key, $json_mods, true ) ? wp_json_encode( $val ) : $val;
				...
				$page = self::get_page_by_path( $custom_layout_slug, OBJECT, 'neve_custom_layouts' );
				if ( $page === null ) {
					continue;
				}
