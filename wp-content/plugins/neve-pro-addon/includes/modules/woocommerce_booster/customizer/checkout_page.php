...
class Checkout_Page extends Base_Customizer {
	...
	private function add_checkout_style() {
		...
		$checkout_background_default = version_compare( NEVE_VERSION, '2.9.0', '<' ) ? '#f7f7f7' : 'var(--nv-light-bg)';
		$this->add_control(
			new Control(
		...
		$box_background_default = version_compare( NEVE_VERSION, '2.9.0', '<' ) ? '#ffffff' : 'var(--nv-site-bg)';
		$this->add_control(
			new Control(
